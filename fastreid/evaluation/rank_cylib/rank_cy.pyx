...
# Main interface
cpdef evaluate_cy(distmat_or_indices, q_pids, g_pids, q_camids, g_camids, max_rank,
                  use_metric_cuhk03=False, use_distmat=True):
    if use_distmat:
        indices = np.argsort(np.asarray(distmat_or_indices, dtype=np.float32), axis=1)
    else:
    ...
    if use_metric_cuhk03:
        return eval_cuhk03_cy(indices, q_pids, g_pids, q_camids, g_camids, max_rank)
    return eval_market1501_cy(indices, q_pids, g_pids, q_camids, g_camids, max_rank)


cpdef eval_cuhk03_cy(long[:,:] indices, long[:] q_pids, long[:]g_pids,
...
cpdef eval_market1501_cy(long[:,:] indices, long[:] q_pids, long[:]g_pids,
                         long[:]q_camids, long[:]g_camids, long max_rank):
